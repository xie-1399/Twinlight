...
class FPMULTest extends AnyFunSuite {
  ...
  Seq(true, false).foreach { outputReg =>
    testConfigs.foreach { testConfig =>
      test(s"FPMULTest(is_wallace = ${testConfig._1}, is_bitslice = ${testConfig._2}, outputReg ${if (outputReg) "en" else "dis"}able) random test") {
        SIMCFG().compile {
          val dut = if (tool.IEEE_FP32) {
            FMUL(expWidth = 8, precision = 23, is_wallace = testConfig._1, is_bitslice = testConfig._2, outputReg = outputReg, pipeAt = pipeAt)
          } else {
            FMUL(expWidth = 5, precision = 10, is_wallace = testConfig._1, is_bitslice = testConfig._2, outputReg = outputReg, pipeAt = pipeAt)
          }
          dut
        }.doSimUntilVoid {
          dut =>
