...
class CSA2_2(len: Int) extends CarrySaveAdderMToN(2, 2)(len) {
  val temp = Vec.fill(len)(UInt(2 bits))
  for ((t, i) <- temp.zipWithIndex) {
    val (a, b) = (io.inx(0)(i), io.inx(1)(i))
    val sum = a ^ b
    val cout = a & b
    t := cout.asUInt @@ sum.asUInt
  }
  io.outx.zipWithIndex.foreach({ case (x, i) =>
    x := Cat(temp.reverse.map(_(i))).asUInt
  })
}
...
class CSA3_2(len: Int) extends CarrySaveAdderMToN(3, 2)(len) {
  val temp = Vec.fill(len)(UInt(2 bits))
  for ((t, i) <- temp.zipWithIndex) {
    val (a, b, cin) = (io.inx(0)(i), io.inx(1)(i), io.inx(2)(i))
    val a_xor_b = a ^ b
    val a_and_b = a & b
    val sum = a_xor_b ^ cin
...
class CSA5_3(len: Int) extends CarrySaveAdderMToN(5, 3)(len) {
  ...
  io.outx := Vec(FAs(1).io.outx(0), FAs(0).io.outx(1), FAs(1).io.outx(1))
}
